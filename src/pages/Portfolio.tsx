...
const styles = `
  ...
  .pf-dot-grid { position: fixed; inset: 0; z-index: 0; pointer-events: none; background-image: radial-gradient(rgba(33,150,243,0.055) 1px, transparent 1px); background-size: 36px 36px; }
  ...
  @keyframes pfBubbleRise { 0% { transform: translateY(0) scale(0.6); opacity: 0; } 8% { opacity: 0.9; } 85% { opacity: 0.45; } 100% { transform: translateY(-105vh) scale(1); opacity: 0; } }
  ...
  .pf-badge { display: inline-flex; align-items: center; gap: 8px; padding: 7px 20px; background: rgba(21,101,192,0.15); border: 1px solid rgba(33,150,243,0.35); border-radius: 100px; font-size: 13px; letter-spacing: 0.05em; color: var(--cyan-soft); margin-bottom: 36px; backdrop-filter: blur(12px); }
  ...
  .pf-hero__title { font-family: 'Syne', sans-serif; font-size: clamp(3rem, 7vw, 5.5rem); font-weight: 800; line-height: 1.06; letter-spacing: -0.04em; margin-bottom: 24px; }
  .pf-hero__title .solid { color: var(--white); }
  .pf-hero__title .grad { background: linear-gradient(110deg, #4fc3f7 0%, #1976d2 30%, #00e5ff 65%, #81d4fa 100%); background-size: 220% auto; -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; animation: pfShimmer 4.5s linear infinite; }
  ...
  .pf-filter-btn { padding: 10px 26px; border-radius: 100px; font-family: 'DM Sans', sans-serif; font-size: 13.5px; font-weight: 600; cursor: pointer; transition: all 0.22s; border: 1px solid rgba(33,150,243,0.2); background: rgba(6,21,37,0.5); color: var(--muted); backdrop-filter: blur(8px); }
  .pf-filter-btn:hover { background: rgba(33,150,243,0.1); border-color: rgba(0,229,255,0.35); color: var(--blue-bright); }
  .pf-filter-btn.active { background: linear-gradient(130deg, #0d47a1 0%, #1976d2 50%, #039be5 85%, #29b6f6 100%); border-color: rgba(41,182,246,0.4); color: #fff; box-shadow: 0 4px 18px rgba(13,71,161,0.45); }
  ...
  .pf-card { background: var(--card-bg); border: 1px solid var(--border); border-radius: 20px; overflow: hidden; backdrop-filter: blur(16px); transition: transform 0.35s, border-color 0.35s, box-shadow 0.35s; display: flex; flex-direction: column; }
  .pf-card:hover { transform: translateY(-7px); border-color: rgba(0,229,255,0.4); box-shadow: 0 24px 56px rgba(0,229,255,0.07); }

  .pf-card__img { width: 100%; aspect-ratio: 16/9; overflow: hidden; background: var(--navy-3); position: relative; }
  ...
  .pf-card__title { font-family: 'Syne', sans-serif; font-size: 1.1rem; font-weight: 700; color: var(--white); margin-bottom: 12px; line-height: 1.4; transition: color 0.2s; }
  ...
  .pf-link { display: inline-flex; align-items: center; gap: 6px; font-size: 13px; font-weight: 600; text-decoration: none; padding: 7px 16px; border-radius: 8px; transition: all 0.2s; }
  ...
  .pf-cta__card { max-width: 700px; margin: 0 auto; position: relative; z-index: 1; background: rgba(6,21,37,0.8); border: 1px solid rgba(33,150,243,0.25); border-radius: 24px; padding: 64px 48px; backdrop-filter: blur(20px); box-shadow: 0 0 80px rgba(21,101,192,0.15), inset 0 1px 0 rgba(255,255,255,0.05); }
...
const PROJECTS = [
  ...
  {
    id: 13,
    title: "Portfolio & Agency – Webflow Site",
    description: "A visually rich Webflow site built with CMS-powered collections, custom scroll interactions, and smooth page animations. Converted directly from a Figma design with pixel-perfect accuracy and full client editability.",
    image: "/webflow-portfolio.png",
    tags: ["Webflow", "Webflow CMS", "Interactions", "Figma to Webflow", "Animations", "Responsive Design"],
    category: "Webflow",
    liveUrl: "#",
  },
];
        ...
        <section className="pf-hero">
          <div className="pf-hero__inner">
            ...
            <p className="pf-hero__desc fade-up visible" style={{ transitionDelay: "0.2s" }}>
              A showcase of my projects and applications, demonstrating expertise in Frontend development, WordPress, WooCommerce, and Webflow.
            </p>
          </div>
        </section>
